...
| **Topic** | A predefined civic category such as Immigration, browsable through [SEARCH_API.md](SEARCH_API.md). |
...
- Topic/discussion examples remain separate fixtures. Public exploration can search existing backend opinions through [SEARCH_API.md](SEARCH_API.md); predefined topics are now browsable, while React views for discovered clusters and position analysis remain deferred. The former session/report frontend has been removed; it creates no additional API requirements.
