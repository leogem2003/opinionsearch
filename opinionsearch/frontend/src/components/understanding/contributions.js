...
export async function createContribution(input) {
  const result = contributionDemo
    ...
    : await requestJSON(BASE, { method: 'POST', body: input })
  if (typeof result.id !== 'string' || !contributionIdPattern.test(result.id) || result.publication !== 'private' || typeof result.accessToken !== 'string' || !result.accessToken) {
...
export async function getContribution(id, { token, signal } = {}) {
  ...
  if (result.id !== id || typeof result.text !== 'string' || result.publication !== 'private' || typeof result.createdAt !== 'string' || !Number.isFinite(Date.parse(result.createdAt))) {
