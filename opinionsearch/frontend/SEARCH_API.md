...
- `query`: a topic or statement, trimmed; at most 2,000 Unicode code points, without NUL. Empty input returns an empty list without loading the model or querying the database. Invalid input returns 400.
...
- Results are ordered by increasing cosine distance and limited to 50. The limit bounds the response, not the number of opinions in the database.
...
Use the same-origin `/api` proxy setup in [README.md](README.md). The [contribution contract](API_CONTRACT.md) describes submission, embedding, sentiment, provenance and retries. Successfully submitted public inputs are searchable; organised discussion visualisations still use explicitly labelled examples.
